...
export default function TeacherMarkbook() {
  ...
  const fetchMarkbookData = async (classId: string) => {
    ...
    try {
      ...
      console.log(classStdsData, stdsError);
  ...
  const handleUpdateGradeRecord = async (
    ...
    const payload: DatabaseGrade & { date_obtained?: string } = {
      ...existingRecord,
      assessment_id: selectedAssessmentId,
      nsn: String(nsn),
      date_obtained: currentDate,
    };
...
const handleAddStandard = async () => {
  const rawInput = newASCode.trim();
  if (!rawInput) return;

  // Extract only digits to match integer standard codes (e.g., "AS91909" -> "91909" or "91909" -> "91909")
  const cleanAS = rawInput.replace(/\D/g, '');

  setStandardError(null);

  try {
    const numericClassId = parseInt(selectedClassId.replace(/\D/g, ''), 10);
    const targetClassId = isNaN(numericClassId) ? selectedClassId : numericClassId;

    // 1. Check for Duplicate Standard in this class
    ...
    if (duplicateError) {
      console.error('Error checking duplicate standard:', duplicateError.message);
      setStandardError('Error verifying existing standards.');
      return;
    }

    if (duplicateCheck) {
      setStandardError(`Standard "${cleanAS}" is already linked to this class.`);
      return;
    }

    // 2. Verify standard exists in master `standards` table
    const { data: existingStandard, error: fetchError } = await supabase
      .from('standards')
      .select('as, standard_name, credits, is_external')
      .eq('as', cleanAS)
      .maybeSingle();

    if (fetchError) {
      console.error('Error checking standard existence:', fetchError.message);
      setStandardError('Error verifying standard code.');
      return;
    }

    if (!existingStandard) {
      setStandardError(`Standard "${cleanAS}" does not exist in the standards database.`);
      return;
    }

    // 3. Automatically derive assessment_type based on activeTab
    ...
    // 4. Insert into `classes_standards`
    ...
    if (insertError) {
      console.error('Failed to link standard to class:', insertError.message);
      setStandardError(insertError.message || 'Failed to add standard to class.');
      return;
    }

    const generatedAssessmentId = String(insertedData.assessment_id);
    ...
    // 5. Update state & UI reset
    setClassStandards((prev) => [...prev, newClassStandard]);
    setSelectedAssessmentId(generatedAssessmentId);
    setNewASCode('');
    setAddStandardModalVisible(false);
  } catch (err) {
    console.error('Error in handleAddStandard:', err);
        ...
        <Modal visible={addStandardModalVisible} transparent animationType="slide">
          <View style={styles.modalOverlay}>
            <View style={[styles.modalCard, { backgroundColor: theme.modalBg, borderColor: theme.border }]}>
              ...
              {/* Auto-detected Assessment Type Indicator */}
              <View style={{ marginBottom: 16 }}>
                <Text style={[styles.inputLabel, { color: theme.textMuted, marginBottom: 2 }]}>
                  Assessment Type
                </Text>
                <Text style={{ color: theme.primary, fontWeight: '600', fontSize: 14 }}>
                  {activeTab === 'internals' || activeTab === 'externals'
                    ? 'Official AS'
                    : activeTab === 'uegs'
                    ? 'UEG'
                    : 'EOTT / Class Test'}
              ...
              <TextInput
                style={[
                  styles.textInput,
                  { color: theme.text, borderColor: standardError ? '#EF4444' : theme.border },
                ]}
                value={newASCode}
                onChangeText={(text) => {
                  setNewASCode(text);
                ...
                placeholder={activeTab === 'classTests' ? 'e.g. 91900 or AS91900' : 'e.g. 91900'}
                placeholderTextColor={theme.textMuted}
                autoCapitalize="characters"
              />

              {/* Validation Error Message with adjusted spacing */}
              {standardError && (
              ...
              {/* Submit Button */}
              <TouchableOpacity
                style={[styles.submitButton, { backgroundColor: theme.primary, marginTop: 16 }]}
